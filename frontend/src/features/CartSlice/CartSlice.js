...
export const saveCart = createAsyncThunk("cart/save", async(cartData) => {
    ...
    const response = await fetch("https://e-commerce-err0.onrender.com/api/cart/save", {
        method:"POST",
...
export const fetchCart = createAsyncThunk("cart/fetch", async(userId) => {
    let token = localStorage.getItem("token")
    const response  = await fetch(`https://e-commerce-err0.onrender.com/api/cart/${userId}`, {
        method: "GET",
        headers:{"Content-Type":"application/json",
        Authorization:`Bearer ${token}`
        },
